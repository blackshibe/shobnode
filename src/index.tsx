...
import DebugTable from "ui/ShobnodeTable";
...
export namespace Shobnode {
	...
	const positioned_nodes: positionedNode[] = [];
	...
	export function setup(target?: Instance) {
		...
		main_node_tree = Roact.mount(
			<Node
				color={configuration.ui_text_color}
				position={configuration.main_node_position[0]}
				data={main_node_data}
			/>,
			ui
		);
	...
	export function display_node(
		...
		let node = positioned_nodes[sign];
		if (node) {
			if (data) {
			...
			const tree = Roact.mount(
				<Node
					anchor_to_middle={anchor_to_middle}
					color={color || configuration.ui_text_color}
					position={position[0]}
					data={data}
				/>,
				ui
	...
	let dont_emit = ["TextScraper text too long"];
	export function log(message: string, message_type: Enum.MessageType) {
		for (const [_, value] of pairs(dont_emit)) if (message.find(value)[0]) return;
		...
		task.delay(configuration.logger_fadeout_time, () => {
			let index = log_info.indexOf(log_entry);
